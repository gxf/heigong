...
&lvl2);
	...
	case  TT_fontname:
	    pap=(PAP *)(mydata->props);
	    wvAppendStr(&mydata->retstring, g_fontname);
	    mydata->currentlen = strlen (mydata->retstring);
	  break;
	case  TT_fontsize:
	    {
		char fs[10];
		sprintf(fs, "%d", g_fontsize);
		wvAppendStr(&mydata->retstring, fs);
	    }
	    mydata->currentlen = strlen (mydata->retstring);
	  break;
...
/*    printf("start: %s \n",name);*/
    ...
    switch (token_type)
	  ...
	  break;
   	  case  TT_fontname:
	  wvAppendStr (mydata->current, "<fontname/>");
	  mydata->currentlen = strlen (*(mydata->current));
	  break;
   	  case  TT_fontsize:
	  wvAppendStr (mydata->current, "<fontsize/>");
	  mydata->currentlen = strlen (*(mydata->current));
	  break;
	  /* --------------<< */
