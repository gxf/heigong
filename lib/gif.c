 ...
 * bug 1: interlace 的图片的bug解决, y+=interlace_rate[pass]的位置放错了
 *
...
static int gifin_get_pixel(int *pel)
{
     ...
     while (pstk_idx == 0)
     {
	  /* load bytes until we have enough bits for another code, code_size是会变化的,最长12 bits */
	  while (work_bits < code_size)
	  {
	       if (buf_idx == buf_cnt)
	       {
		    /* read a new data block, 最多允许有256个bytes */
		    if ((errno = gifin_read_data_block()) != GIFIN_SUCCESS)
			 return errno;
	  ...
	  /* get the next code */
	  code        = work_data & code_mask; // 取得需要的bits
	  work_data >>= code_size; // 去处已经被取得的code
	  work_bits  -= code_size; // 更新work bits

	  /* interpret the code, reset 解码的表*/
	  if (code == clr_code)
	  {
	       /* reset decoder stream */
	       code_size  = root_size + 1; // 设置code size 为原始的root_size +1
	       code_mask  = (1 << code_size) - 1; // 相应的修改code_mask
	       prev_code  = NULL_CODE; 
...
STATUS gifLoad(char *fullname, ImageOptions *image_ops)
{
     ...
     /* 使用local colormap 还是 global colormap */
     if (gifin_l_cmap_flag) {
